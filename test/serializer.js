...
import JSONAPISerializer from "../lib/serializer.mjs";
...
describe("Options", function () {
  describe("id", function () {
    it("should override the id field", function (done) {
      ...
      const json = new JSONAPISerializer("users", {
        id: "_id",
    ...
    it("should not be serialized when it's null", function (done) {
      ...
      const json = new JSONAPISerializer("user", {
        attributes: ["firstName", "lastName"],
      }).serialize(dataSet);
  ...
  describe("typeForAttribute", function () {
    ...
    it("should pass the object as a second constiable to the func", function (done) {
      const dataSet = {
        id: "1",
        firstName: "Sandro",
        lastName: "Munda",
  ...
  describe("dataMeta option", function () {
    ...
    it("should set the meta according to the func return", function (done) {
      ...
      const json = new JSONAPISerializer("user", {
        attributes: ["firstName", "lastName"],
        dataMeta: function (record) {
          return { copyright: record.firstName + " " + record.lastName };
        },
      }).serialize(dataSet);
  ...
  describe("keyForAttribute case strings", function () {
    ...
    it("should default the key case to dash-case", function (done) {
      const jsonNoCase = new JSONAPISerializer("user", dataSet, {
        attributes: ["firstName"],
      });

      const jsonInvalidCase = new JSONAPISerializer("user", {
        attributes: ["firstName"],
    ...
    it("should update the key case to dash-case", function (done) {
      const jsonDashCase = new JSONAPISerializer("user", {
        attributes: ["firstName"],
        keyForAttribute: "dash-case",
      }).serialize(dataSet);

      const jsonLispCase = new JSONAPISerializer("user", {
        attributes: ["firstName"],
        keyForAttribute: "lisp-case",
      }).serialize(dataSet);

      const jsonSpinalCase = new JSONAPISerializer("user", {
        attributes: ["firstName"],
        keyForAttribute: "spinal-case",
      }).serialize(dataSet);

      const jsonKababCase = new JSONAPISerializer("user", {
        attributes: ["firstName"],
    ...
    it("should update the key case to underscore_case", function (done) {
      const jsonUnderscoreCase = new JSONAPISerializer("user", {
        attributes: ["firstName"],
        keyForAttribute: "underscore_case",
      }).serialize(dataSet);

      const jsonSnakeCase = new JSONAPISerializer("user", {
        attributes: ["firstName"],
    ...
    it("should update the key case to CamelCase", function (done) {
      ...
      const jsonCamelCase = new JSONAPISerializer("user", {
        attributes: ["firstName"],
...
describe("JSON API Serializer", function () {
  ...
  describe("Nested document", function () {
    ...
    it("should not contains attributes key", function (done) {
      const dataSet = [{ id: 2 }, { id: 3 }];

      const json = new JSONAPISerializer("tags", {
        ref: true,
        included: false,
        topLevelLinks: {
  ...
  describe("Compound document", function () {
    it("should be set into the `data.relationships` and `included`", function (done) {
      ...
      expect(json.included[1]).to.have.property("id").equal("123");
  ...
  describe("Cycling references", function () {
    it("should set the relationships in both includes", function (done) {
      ...
      const json = new JSONAPISerializer("cycling", {
        attributes: ["user"],
        user: {
          ref: "id",
  ...
  describe("Database model objects", function () {
    it("properly pass on requests for attributes", function () {
      ...
      const user = new User({ firstName: "Lawrence", lastName: "Bennett" });
  ...
  describe("falsy attribute values", function () {
    it("properly attach falsy attributes", function () {
      ...
      const json = new JSONAPISerializer("tester", {
    ...
    it("should set the relationshipLinks parameter when the nullIfMissing is used", function () {
      ...
      expect(json.included).eql([
        ...
        },
        {
          ...
          relationships: {
            publisher: { data: null, links: { related: "bar" } },
          },
        },
      ]);
    });
