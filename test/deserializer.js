...
import JSONAPIDeserializer from "../lib/deserializer.mjs";
...
describe("JSON API Deserializer", function () {
  ...
  describe("Nested documents", function () {
    it("should returns attributes", function (done) {
      ...
      new JSONAPIDeserializer({ keyForAttribute: "camelCase" }).deserialize(
        dataSet,
        function (err, json) {
          expect(json).to.be.an("array").with.length(2);
          ...
          expect(json[1]).to.have.key("id", "firstName", "lastName", "books");
          done(null, json);
        }
      );
    });
  ...
  describe("Compound document", function () {
    ...
    describe("With polymorphic relationships to same records", function () {
      it("should return all data without circular error", function (done) {
        const imageOne =
          "https://avatars2.githubusercontent.com/u/15112077?s=400&u=9860ca2648dd28ec2c726d287980b4f7d615f590&v=4";
        const imageTwo =
          "https://www.placewise.com/images/employees/ashley-schauer.jpg";
        const dataSet = {
          data: {
            id: "1",
            type: "users",
                ...
                deals: [
                  {
                    name: "Buy One Get One Off Shirts",
                    id: "3",
                    stores: [{ name: "Fashionable Clothes", id: "2" }],
                  },
                ],
              },
              {
                ...
                stores: [
                  {
                    name: "Fashionable Clothes",
                    id: "2",
                    deals: [{ name: "Buy One Get One Off Shirts", id: "3" }],
                  },
                ],
              },
            ],
    ...
    describe("when mixed collection with option to include type as attributes", function () {
      it("should include type as key", function (done) {
        ...
        new JSONAPIDeserializer({ typeAsAttribute: true }).deserialize(
          dataSet,
          function (err, json) {
            expect(json).to.be.an("array").with.length(2);
